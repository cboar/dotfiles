...
syn match   jadeBegin "^\s*\%([<>]\|&[^=~ ]\)\@!" nextgroup=jadeTag,jadeClassChar,jadeIdChar,jadePlainChar,jadeJavascript,jadeScriptStatement,jadePipedText
...
syn region jadeJavascript matchgroup=jadeScriptConditional start="else" end="$" contained
syn region jadeJavascript matchgroup=jadeScriptConditional start="else if\|if\|while\|unless\|until\|case\|when\|default" end="$" contained contains=@htmlJavascript keepend
...
hi def link jadeId                     Special
hi def link jadeClass                  Special
