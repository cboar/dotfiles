...
syn keyword cssTagName progress time rt rp section time video filter
...
syn match cssFuncVal "\<\%\(\w\|_\|-\)*("me=e-1
...
" Box
syn match cssBoxProp /flex-\(direction\|grow\|wrap\|shrink\|basis\|flow\)/ contained
...
syn keyword cssTextVal left right center justify initial inherit contained
...
syn keyword cssCommonProp content quotes filter contained
...
syn keyword cssAdvancedProp phonemes resize richness size volumne src
...
if version >= 508 || !exists("did_css_syn_inits")
  ...
  HiLink cssSpecialProp StorageClass
  HiLink cssImportant Special
  HiLink cssRuleProp PreProc
  HiLink cssPseudo Special

  HiLink cssColorVal Constant
  HiLink cssCommonVal PreProc
  HiLink cssFontVal PreProc
  HiLink cssListVal PreProc
  HiLink cssTextVal PreProc
  HiLink cssVisualVal PreProc
  HiLink cssBorderVal PreProc
  HiLink cssBackgroundVal PreProc
...
hi def link stylusAmpersand             Statement
hi def link stylusClass					Special
hi def link stylusClassChar             Special
hi def link stylusEscape                Special
hi def link stylusId                    Special
